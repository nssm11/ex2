  ...
  return (
    <>
      <PageIntro
        ...
        title={<>{c.name} <em className="text-vert">· sélection</em></>}
        intro={c.description}
      ...
      >
        {siblings.length > 0 && (
          <div className="mt-10 flex flex-wrap items-center gap-x-7 gap-y-3 border-t border-stone pt-6">
            <span className="text-micro font-semibold tracking-[0.08em] text-muted">Dans {c.parent?.name}</span>
            {siblings.map((s) => (
              s.slug === c.slug
                ? <span key={s.id} className="border-b border-sage pb-1 text-sm text-vert" aria-current="page">{s.name}</span>
