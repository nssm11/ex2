  ...
  return (
    <div className="container-lux py-14 lg:py-20">
      <div className="mx-auto max-w-2xl text-center">
        <Reveal y={0}><span className="mx-auto flex h-14 w-14 items-center justify-center border border-sage text-vert"><CheckIcon size={24} /></span></Reveal>
          ...
          <ul className="mb-10 divide-y divide-stone border-y border-stone" aria-label="Articles commandés">
            {o.items.map((i) => (
              <li key={i.id} className="flex gap-4 py-4">
                <div className="relative aspect-square w-16 shrink-0 overflow-hidden rounded-sm bg-paper-2">{i.image && <Image src={i.image} alt="" fill sizes="64px" className="object-cover" />}</div>
                <div className="min-w-0 flex-1">
                  <p className="text-micro font-semibold tracking-[0.08em] text-sage-2">{i.brandName}</p>
                  <p className="font-display text-[15px] leading-snug text-ink">{i.name}</p>
