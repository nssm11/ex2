    ...
    <div className="border-b border-stone">
      <section className="bg-vert text-cream">
        <div className="container-lux py-12 lg:py-16">
          <p className="eyebrow mb-6 flex items-center gap-3 text-cream/55"><span className="font-display text-lg italic text-sage-3">Recherche</span> L&apos;archive Cléopâtre</p>
          <h1 className="font-display text-display-lg">
            {q ? <>«&nbsp;<em className="text-sage-3">{q}</em>&nbsp;»</> : <>Que cherchez-vous&nbsp;?</>}
          </h1>
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-cream/60">{q ? "Voici les produits qui correspondent le mieux à votre recherche." : "Un actif, une marque, un besoin : cherchez dans toute la sélection — produits, marques, univers et conseils."}</p>
        ...
        <div className="container-lux pb-16">
          <div className="border border-dashed border-stone-2 bg-cream px-6 py-14 text-center">
            ...
            <ul className="mt-7 flex flex-wrap justify-center gap-2">
              {unis.map((u) => <li key={u.id}><Link href={`/univers/${u.slug}`} className="inline-flex min-h-10 items-center border border-stone-2 px-4 text-micro font-semibold tracking-[0.08em] text-charcoal transition-colors hover:border-vert hover:bg-vert hover:text-cream">{u.name}</Link></li>)}
            </ul>
          </div>
        </div>
      )}
