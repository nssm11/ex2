    ...
    <AdminPage title={`${u.firstName} ${u.lastName}`} sub={u.email} action={me?.role === "admin" ? <RoleSelect userId={u.id} role={u.role} /> : undefined}>
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          <Panel className="p-5 text-sm"><h2 className="mb-3 text-[10px] tracking-[0.02em] text-admin-muted">Profil</h2><p>{u.phone ?? "—"}</p><p className="text-admin-muted">Inscrit le {formatDate(u.createdAt)}</p><p className="mt-2">{u.loyaltyPoints} points fidélité</p></Panel>
          <Panel className="p-5 text-sm"><h2 className="mb-3 text-[10px] tracking-[0.02em] text-admin-muted">Adresses</h2>{addr.length === 0 ? <p className="text-admin-muted">Aucune.</p> : <ul className="space-y-3">{addr.map((a) => <li key={a.id}><p>{a.label}</p><p className="text-admin-muted">{a.line1}, {a.city}, {a.governorate}</p></li>)}</ul>}</Panel>
          <Panel className="p-5"><h2 className="mb-3 text-[10px] tracking-[0.02em] text-admin-muted">Notes internes</h2><CustomerNote userId={u.id} notes={u.notes ?? ""} /></Panel>
        </div>
      </div>
    </AdminPage>
  );
