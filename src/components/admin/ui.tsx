  ...
  return (
    <div>
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4 border-b border-admin-border pb-6">
        <div>
          {eyebrow && <p className="mb-2 text-[9px] font-bold tracking-[0.02em] text-admin-gold">{eyebrow}</p>}
    ...
    <div className={cn("border border-admin-border bg-admin-panel", className)}>
      {title && (
        <div className="flex items-center justify-between gap-4 border-b border-admin-border px-5 py-3.5">
          <h2 className="text-[10px] font-bold tracking-[0.02em] text-admin-muted">{title}</h2>
          {action}
        </div>
      )}
      {children}
  ...
  return (
    <div className="overflow-x-auto border border-admin-border bg-admin-panel">
      <table className={cn("w-full border-collapse text-[13px]", minWidth)}>
        <thead>
          <tr className="border-b border-admin-border text-left text-[9px] font-bold tracking-[0.02em] text-admin-muted">
...
export function StatusBadge({ s, className }: { s: OrderStatus; className?: string }) {
  return <span className={cn("inline-flex whitespace-nowrap px-2.5 py-1 text-[9px] font-bold tracking-[0.02em]", tones[s], className)}>{ORDER_STATUS_LABELS[s]}</span>;
}
...
export function KPI({ label, value, sub, tone = "text-admin-text" }: { label: string; value: string; sub?: string; tone?: string }) {
  return (
    <div className="relative overflow-hidden border border-admin-border bg-admin-panel px-5 py-6">
      ...
      <p className="text-[9px] font-bold tracking-[0.02em] text-admin-muted">{label}</p>
...
export const abtn = "inline-flex min-h-11 items-center justify-center gap-2 bg-admin-gold px-4 text-[11px] font-bold tracking-[0.02em] text-noir transition-colors hover:bg-admin-text disabled:opacity-40";
export const abtnGhost = "inline-flex min-h-11 items-center justify-center gap-2 border border-admin-border px-4 text-[11px] font-bold tracking-[0.02em] text-admin-text transition-colors hover:border-admin-gold hover:text-admin-gold disabled:opacity-40";
export const abtnDanger = "inline-flex min-h-11 items-center justify-center gap-2 border border-error px-4 text-[11px] font-bold tracking-[0.02em] text-error transition-colors hover:bg-error hover:text-admin-text disabled:opacity-40";

export function AField({ label, children, error, hint }: { label: string; children: ReactNode; error?: string; hint?: string }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-[9px] font-bold tracking-[0.02em] text-admin-muted">{label}</span>
      {children}
...
export function SectionLabel({ children, className }: { children: ReactNode; className?: string }) {
  return <h2 className={cn("mb-3 text-[10px] font-bold tracking-[0.02em] text-admin-muted", className)}>{children}</h2>;
}
