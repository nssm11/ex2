...
export function AuthShell({ title, kicker, children }: { title: ReactNode; kicker: string; children: ReactNode }) {
  return (
    <div className="grid min-h-dvh lg:grid-cols-2">
      {/* Editorial panel */}
      <div className="relative hidden overflow-hidden bg-vert text-cream lg:block">
        ...
        <div className="relative flex h-full flex-col justify-between p-12 xl:p-16">
          <Link href="/" className="flex items-center gap-3 text-cream">
            ...
            <span className="flex flex-col leading-none">
              ...
              <span className="mt-1 text-[8px] font-bold tracking-[0.02em] text-cream/50">Espace Santé Beauté</span>
            </span>
          </Link>
          <div className="max-w-md">
            <p className="eyebrow mb-6 text-sage-3">L&apos;espace client</p>
            ...
            <ul className="mt-10 space-y-4 text-sm text-cream/70">
              <li className="flex items-center gap-3"><TruckIcon size={16} className="shrink-0 text-sage-3" /> Suivi de commande 24–72 h partout en Tunisie</li>
              <li className="flex items-center gap-3"><StoreIcon size={16} className="shrink-0 text-sage-3" /> Retrait 2 h à Ezzahra ou Hammam-Lif</li>
              <li className="flex items-center gap-3"><ShieldIcon size={16} className="shrink-0 text-sage-3" /> Produits authentiques, conseil de pharmacien</li>
            </ul>
          </div>
          <p className="text-xs text-cream/40">© {new Date().getFullYear()} Cléopâtre — Espace Santé Beauté</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center bg-paper px-5 py-14 sm:px-10">
        <div className="w-full max-w-md">
          <div className="mb-8 lg:hidden">
            <Link href="/" className="flex items-center gap-3 text-ink"><LogoMark size={28} className="text-vert" /><span className="font-display text-xl">Cléopâtre</span></Link>
          </div>
