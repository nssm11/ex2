  ...
  return (
    <div>
      {terminal ? <p className="mb-6 inline-flex bg-error-soft px-3 py-1.5 text-xs tracking-[0.02em] text-error">{ORDER_STATUS_LABELS[status]}</p> : (
        <ol className="no-scrollbar-x -mx-1 flex gap-1 overflow-x-auto px-1 pb-1 sm:grid sm:grid-cols-5 sm:overflow-visible" aria-label="Progression">
          {ORDER_FLOW.map((s, i) => (
            <li key={s} className="flex min-w-[68px] shrink-0 flex-col items-center text-center sm:min-w-0">
              <motion.span initial={reduce ? false : { scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: i * 0.12, duration: 0.6, ease: EASE_LUXE }} className={`flex h-8 w-8 items-center justify-center rounded-pill border transition-colors duration-500 ${i <= idx ? "border-vert bg-vert text-cream" : "border-stone-2 text-muted-2"}`}>{i < idx ? <CheckIcon size={14} /> : <span className="text-[11px]">{i + 1}</span>}</motion.span>
              <span className={`mt-2 text-micro tracking-[0.08em] ${i <= idx ? "text-ink" : "text-muted-2"}`}>{ORDER_STATUS_LABELS[s]}</span>
            </li>
          ))}
        </ol>
      )}
      <ul className="mt-8 space-y-4 border-l border-stone pl-5">
        {events.map((e, i) => (
          <motion.li key={i} initial={reduce ? false : { opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.08, duration: 0.6, ease: EASE_LUXE }} className="relative text-sm">
            <span className="absolute -left-[23px] top-1.5 h-1.5 w-1.5 bg-vert" />
